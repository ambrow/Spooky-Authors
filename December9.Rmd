---
title: "Spooky Authors Kaggle"
author: "Ricky Tharrington and Alec Brown"
...
```{r}
train = read.csv('C:\\Users\\Alec\\Desktop\\Kaggle\\Spooky Authors (Ricky)\\train.csv', 
                 colClasses = c('factor','character','factor'))
test = read.csv('C:\\Users\\Alec\\Desktop\\Kaggle\\Spooky Authors (Ricky)\\test.csv', 
...
save(list = c('nb_train','nb_test'), file = 'C:\Users\Alec\Desktop\Kaggle\Spooky Authors (Ricky)\NBResults.Rdata')
```
...
for (i in 1:10){
  ...
  valid.preds[i] = list(matrix(results$p_valid,byrow = T,ncol = 3))
  valid.ids = train.m.full[cross_val==i,]@Dimnames[[1]][[i]]
}
```

##collect cross validation predictions and match to actual value

```{r}
collection=cbind(train[cross_val==1,]$author,valid.preds[[1]])
for(i in 2:10){
  collection=rbind(collection,cbind(train[cross_val==i,]$author,valid.preds[[i]]))
}
collection=data.frame(collection)
names(collection) = c('author','EAP','HPL','MWS')
collection$author=as.factor(collection$author)
```

##SVM model off of predicted probabilities
```{r}
svm.model = tune.svm(author~., data = collection[cross_val!=1,], gamma = 2^(-1:1), cost = 2^(2:4),validation.x=collection[cross_val==1,])
```
##best parameters: gamma=.5, cost =8
#retrain to get probabilities for submission
```{r}
svm.best = svm(author~.,data=collection,gamma=.5,cost=8,probability=TRUE)
```
## new submission
```{r}
new_preds = Reduce("+",test.preds)/length(test.preds)
new_preds=data.frame(new_preds)
names(new_preds)=c('EAP','HPL','MWS')
test_svm = predict(svm.best,newdata=new_preds, probability = TRUE)
test_svm_preds=cbind(as.character(test$id),attr(test_svm,"probabilities"))
test_svm_preds=data.frame(test_svm_preds)
names(test_svm_preds)=c('id','EAP','MWS','HPL')
test_svm_preds = select(test_svm_preds,c(1,2,4,3))
write.csv(test_svm_preds,file='svmsubmission.csv',row.names=F)
```
## Creating Submission
...
write.csv(test_scored, file = 'submission.csv', row.names = F)
